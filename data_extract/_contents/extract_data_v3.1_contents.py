...
v3 차이 (vs v2):
  · main 의 site loop 를 ThreadPoolExecutor 로 병렬화 (각 site csv 따로 떨어져 충돌 없음)
  · 새 CLI 옵션 --site-workers <N>  (default 1 = v2 동일 순차)
  · 주의: 동시 API 요청 = site_workers × workers. 너무 크면 AA throttling — 점진 증가 권장.
...
site × panel 룰 (--include-global-for-us 로 us_old 의 [Global] 추출 토글):
  ...
  ※ 2026-05-19 부로 US 의 RSID 가 sscompany_namenewus → sscompany_name4newus 로 갈렸고,
...
def main() -> int:
    ...
    parser.add_argument("--workers", type=int, default=MAX_WORKERS, help=f"site 안 reportlet 병렬 워커 수 (default {MAX_WORKERS})")
