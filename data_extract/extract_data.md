...
# extract_data_v3.1.py — SKIP_PANEL_SEGMENTS 옵션 (기존 패널 세그 무시)

v3 베이스 + **panel 의 segmentGroups (AA Workspace 패널 상단에 박힌 기존 세그) 를 globalFilter 에 포함시킬지 결정하는 토글**. 캠페인마다 패널 세그 일부를 빼고 EXTRA_SEGMENTS 로 다른 세그로 갈아끼우는 시나리오에 사용.

`SKIP_PANEL_SEGMENTS = False` (default) 면 v3 와 100% 동일 동작 (옵트인).

## v3 대비 동작 차이

| 항목 | v3 | v3.1 |
|---|---|---|
| globalFilters 구성 | panel.segmentGroups + EXTRA_SEGMENTS + dateRange | (조건부) panel.segmentGroups + EXTRA_SEGMENTS + dateRange |
| 신규 옵션 | — | `SKIP_PANEL_SEGMENTS: bool \| list[str] = False` |
| 동작 분기 | panel 별 dateRange/추가 segment | + panel.name 키워드 매칭 시 기존 세그 무시 |

## SKIP_PANEL_SEGMENTS 설정 (상단 상수)
...
| 값 | 동작 |
|---|---|
| `False` (default) | 기존 동작 (panel.segmentGroups + EXTRA_SEGMENTS 누적) |
| `True` | **모든 panel** 에서 기존 세그 무시 (EXTRA_SEGMENTS + dateRange 만으로 구성) |
| `["키워드1", "키워드2", ...]` | panel.name 에 키워드 포함된 panel 만 기존 세그 무시 (OR 매칭, case-insensitive) |

예:
```python
SKIP_PANEL_SEGMENTS = True              # 전체 panel 적용
SKIP_PANEL_SEGMENTS = ["[US]"]          # [US] panel 만 적용
SKIP_PANEL_SEGMENTS = ["Revisit", "EPP"]  # Revisit OR EPP 포함된 panel 만 적용
```

## 사용 시나리오

기존 패널에 박힌 globalFilter 세그 (예: `[Global] Excluded EPP`) 를 빼고 다른 세그 (예: `[US] Excluded EPP`) 로 갈아끼우고 싶을 때:

```python
SKIP_PANEL_SEGMENTS = True   # 기존 패널 세그 무시
EXTRA_SEGMENTS = [
...
## EXTRA_SEGMENTS 예시 (v3.1 추가)

기존 v3 예시에 [US] Excluded EPP / [US] Excluded APP 가 commented-out 예시로 추가됨:
```python
EXTRA_SEGMENTS = [
    # 예시 — ID 직접 지정
    # {"segment_id": "세그먼트_아이디_넘버", "panel_scope": "all"},  # visitor id = d=mid, null (Exclude)
    # {"segment_id": "세그먼트_아이디_넘버", "panel_scope": "all"},  # [part_name] Excluded EPP
    # {"segment_id": "세그먼트_아이디_넘버", "panel_scope": "all"},  # [Global] Excluded EPP
    # {"segment_id": "세그먼트_아이디_넘버", "panel_scope": "all"},  # [US] Excluded EPP
    # {"segment_id": "세그먼트_아이디_넘버", "panel_scope": "all"},  # [US] Excluded APP
]
```
...
1. v3 에서 EXTRA_SEGMENTS 만 사용해도 충분하면 그대로 진행
