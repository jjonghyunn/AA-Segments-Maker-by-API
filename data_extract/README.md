...
> **`_contents` 시리즈 주**: contents 계열은 별개 lineage 로, 추출기가 메인(`v4.3`)보다 뒤처진 `v3.2` 이며 입력도 구 명명 `column_mapping_*.csv` 를 사용한다. 메인 계열과 통합되지 않은 채 분기·정체된 상태이며, 추출기 전용 문서는 없다(정제기 md 로 흐름 파악).
