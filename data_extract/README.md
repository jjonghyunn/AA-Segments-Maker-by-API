...
| 도구 | 문서 | 용도 |
|---|---|---|
| `extract_data_v4.5.py` | [`extract_data.md`](extract_data.md) | **현행 추출기** — N단계 breakdown + device 컬럼 + 사이트별 RSID/dateRange override + site 병렬(`SITE_WORKERS`) + **(v4.5) prior 기간 자동계산(`PRIOR_OFFSETS`)** |
...
| `app_O_X_example.csv` | 입력 형식 예시 — site 별 App 론치 O/X (`extract_data_v4.5.py` 의 `requires_app` device 케이스 필터용) |
...
> 구버전 `extract_data_v1~v3.x` 및 그 문서는 삭제됨(2026-06-17). 이력은 git history 참조. 현행은 `extract_data_v4.5.py` 단일 계열.
