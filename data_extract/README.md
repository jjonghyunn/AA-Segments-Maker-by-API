...
| 도구 | 문서 | 용도 |
|---|---|---|
| `extract_data_v5.0.py` | [`extract_data.md`](extract_data.md) | **현행 추출기** — N단계 breakdown + device 컬럼 + 사이트별 RSID/dateRange override + site 병렬(`SITE_WORKERS`) + **(v4.5) prior 기간 자동계산(`PRIOR_OFFSETS`)** + **(v4.6) 작년 기간 수동 지정(`last_year_start`/`last_year_end`)** + **(v5.0) 기간 분할 `PERIOD_SPLIT` — monthly / daily** |
...
> 구버전 `extract_data_v1~v3.x` 및 그 문서는 삭제됨(2026-06-17). 이력은 git history 참조. 현행은 `extract_data_v5.0.py` 단일 계열.
