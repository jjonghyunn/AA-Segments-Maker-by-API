...
| 도구 | 문서 | 용도 |
|---|---|---|
| `extract_data_v4.6.py` | [`extract_data.md`](extract_data.md) | **현행 추출기** — N단계 breakdown + device 컬럼 + 사이트별 RSID/dateRange override + site 병렬(`SITE_WORKERS`) + **(v4.5) prior 기간 자동계산(`PRIOR_OFFSETS`)** + **(v4.6) 작년 기간 수동 지정(`last_year_start`/`last_year_end`)** |
| `RESHAPE_standard_v1.9.py` | [`RESHAPE_standard_v1.9.md`](RESHAPE_standard_v1.9.md) | 추출본 union 정제 — **(v1.9) `period_type`(prior) passthrough + 환율 미조회 경고** + breakdown 행 모드 + `_old` SITE CODE 정규화 + metric/Panel name 컬럼 + product 카테고리 분류 |
...
| `app_O_X_example.csv` | 입력 형식 예시 — site 별 App 론치 O/X (`extract_data_v4.6.py` 의 `requires_app` device 케이스 필터용) |
...
> 구버전 `extract_data_v1~v3.x` 및 그 문서는 삭제됨(2026-06-17). 이력은 git history 참조. 현행은 `extract_data_v4.6.py` 단일 계열.
