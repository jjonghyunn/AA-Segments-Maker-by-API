...
# updated: 2026-05-21  — v1.1: RESHAPE_ prefix 적용, ITEM 추출 조건에 "Recommended for You -" OR 매칭 + 출력 라벨 "Recommended for You -" 로 통일
"""
...
def extract_item(segments: str, metric: str, reportlet: str) -> str | None:
    ...
    # v1.1: CC_ 없지만 "Product Recommendation -" / "Recommended for You -" 패턴 → 그대로 ITEM 으로 사용
    #   Visit 테이블의 개별 recomm 세그먼트에 CC_ prefix 가 없는 구조 대응
    if "Product Recommendation" in last or "Recommended for You" in last:
        s = RE_CAMPAIGN_PREFIX.sub("", last, count=1).strip()
        # [part_name] US_ 등 prefix 제거
        s = re.sub(r"^(US_)?", "", s).strip()
        while RE_TRAILING_PAREN.search(s):
            s = RE_TRAILING_PAREN.sub("", s).strip()
        # 출력 라벨 통일: "Product Recommendation -" → "Recommended for You -"
        s = s.replace("Product Recommendation -", "Recommended for You -")
        return s
