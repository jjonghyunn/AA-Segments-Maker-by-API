...
├── data_extract/           # Workspace 리포트 데이터 추출 (extract_data_v3.7.py 권장 + _contents 등 캠페인 variant)
│   ├── extract_data_v3.7.py        (사이트별 RSID + dateRange override + N단계 breakdown + device 컬럼 + site 병렬)
...
- **v3 → v3.7 (권장)** (`extract_data_v3.7.py`) — v2 의 `sites_input` 기반 멀티사이트 구조 유지 + `site_registry.py` 로 site_code ↔ RSID 매핑 분리. v3.5: **N단계 dimension breakdown** (행 item 을 하위 차원으로 재귀 분해, `bd{k}_*` 컬럼) + **device 컬럼** (컬럼 세그명에서 Mobile/PC/Android/iOS/App 추출). v3.6: **site 단위 병렬** (`SITE_WORKERS`, 기본 5곳 동시, `--site-workers`). v3.7: **레벨별 limit cap** (`LIMIT_LV1`/`LIMIT_BD`) + 출력 2종 개편 (`stack_data_extract_*` / `table_data_extract_*`)
