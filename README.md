...
│   ├── RESHAPE_standard_v1.2.py    (추출본 union 정제 — breakdown 행 모드 + device/bd passthrough + metric/Panel name 컬럼)
...
- **정제** (`RESHAPE_standard_v1.2.py`) — 추출본 union 정제. breakdown 행 모드(`include`/`exclude`/`only`) + device/bd passthrough + `_old` SITE CODE 정규화 + v1.2: metric / Panel name 출력 컬럼
