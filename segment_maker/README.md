...
`--apply` 안 줘도 자동 생성: `segment_v2_result_<ts>_dryrun.csv` (Name / Mode / ParseStatus / Error). 어떤 row 가 어떤 에러인지 한 눈에 식별.
...
aa_create_segment_v2.4.py --input segments.csv
   ↓ dry-run → segment_v2_result_<ts>_dryrun.csv
   ↓ --apply → AA POST/PUT + segment_v2_result_<ts>.csv (형식 예: segments_result_example.csv)
```
...
> ⚠ create 출력 파일(`segment_v2_result_*.csv`)은 `segment_` 로 시작해 이 자동 선택(`result_*` glob)에 **안 잡힌다** — create → delete 를 이어 돌릴 땐 `--from-csv` 로 파일을 직접 지정할 것.
