# example_segment_complex.py
# 2026-05-04  Jonghyun Park w/ Claude
"""
실제 어도비에 만들어져있던 segment 1개를 가져와 sanitize한 예시 — 그대로 가져와
aa_create_segment.py의 SEGMENT_DEFINITION에 복붙 가능.

원본 구조에서 발견한 패턴:
  Hit 단위에서, 다음 두 조건 중 하나(OR)에 해당하는 hit:
    [A] Page에 특정 키워드 포함 AND Page에 [exclusion 리스트] 어느 것도 포함 안 함
    [B] prop29(URL without Parameter)에 같은 키워드 포함 AND prop29에 같은 exclusion 어느 것도 포함 안 함
...
# 실제 사용 시 keyword와 exclusion 리스트를 본인 케이스에 맞게 수정.
SEGMENT_DEFINITION = {
    "func": "segment",
    "version": [1, 0, 0],
    "container": {
...
#   3. python aa_create_segment.py        ← dry-run으로 payload 확인
#   4. python aa_create_segment.py --apply ← 문제 없으면 실제 POST
